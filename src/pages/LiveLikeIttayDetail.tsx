...
const LiveLikeIttalyDetail: React.FC = () => {
  const navigate = useNavigate();
  const params = useParams();
  const { liveittalyId } = params;
  ...
  const handleAddForm = async () => {
    ...
    if (addResult.result && addResult.status === 200) {
      alert("live like ittaly 수정이 완료되었습니다.");
    }
    navigate(-1);
  };
    ...
    <div>
      <div className="flex align-c justify-sb pb-30">
        <div className="flex alicn-c">
          ...
          <p className="page-title mt-3">#Live like ittaly 수정</p>
        </div>
...
export default LiveLikeIttalyDetail;
