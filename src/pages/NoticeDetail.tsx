...
const NoticeDetail = () => {
  ...
  const init = async () => {
    ...
    if (data[0].category === "이벤트") {
      const eventData: any = await getDatas({
        collection: "events",
        find: { _id: data[0]?.targetEventId },
      });

      setSelectedEvent({
        value: eventData.data[0]?.title,
        label: eventData.data[0]?.title,
        _id: eventData.data[0]?._id,
      });
    }
    ...
    setValue(data[0]?.contents);
    setFile({
      file: null,
      fileUrl: data[0]?.imgUrl,
    });
    setOpeningDate(openingDate ? timeFormat2(data[0]?.openingDate) : "");
    setOpenStatus(data[0]?.openStatus ? true : false);
  };
